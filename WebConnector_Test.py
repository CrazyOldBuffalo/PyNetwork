...
def ConnDisconn():
    ...
    if(Connected() == True):
        ...
        while Discloop == True:
            try:
                Disc = input("Do you Want to Disconnect?    Y/N \n")
                Disc = str(Disc)
                if (Disc == 'Y' or Disc == 'y'):
                    print("Disconnecting......")
                    Disconnecting = subprocess.run(['netsh', 'wlan', 'disconnect'], capture_output=True, text=True).stdout
                    print(Disconnecting)
                    ...
                    print(results)
                    #-------------------------------------------
                    Connect = input("Enter the name of the connection you want to connect to: ")
                    Connecting = subprocess.run(['netsh', 'wlan', 'connect', 'name=', Connect],capture_output=True, text=True)
                    print("Connecting.........")
                    if (Connecting.returncode == 0):
                        print("Connection Succcessful!")
                    else:
                        print("Connection Failed")
                elif (Disc == 'N' or Disc == 'n'):
